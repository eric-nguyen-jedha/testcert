...
FRAUD_CSV_URL = 'https://lead-program-assets.s3.eu-west-3.amazonaws.com/M05-Projects/fraudTest.csv'
...
def download_fraud_csv_from_url(**context):
    """Télécharge le fichier CSV depuis une URL et enregistre le chemin dans XCom."""
    logger.info("⬇️ Downloading fraud data CSV from URL...")
    os.makedirs(DATA_PATH, exist_ok=True)
    local_filename = os.path.join(DATA_PATH, "fraudTest.csv")

    try:
        response = requests.get(FRAUD_CSV_URL)
        response.raise_for_status()
        with open(local_filename, 'wb') as f:
            f.write(response.content)
        logger.info(f"✓ CSV downloaded: {local_filename}")
        context["ti"].xcom_push(key="local_fraud_csv", value=local_filename)
    except Exception as e:
...
def evidently_data_quality_check(**context):
    ...
    csv_path = context["ti"].xcom_pull(task_ids="download_fraud_csv", key="local_fraud_csv")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Input CSV not found: {csv_path}")

    fraud_df = pd.read_csv(csv_path)
    split_idx = len(fraud_df) // 2
    reference_data, current_data = fraud_df.iloc[:split_idx], fraud_df.iloc[split_idx:]

    numerical_columns = [c for c in fraud_df.select_dtypes(include=['float64', 'int64']).columns]
    ...
    report_path = os.path.join(REPORTS_PATH, "fraud_drift_report.html")
    ...
    test_suite_path = os.path.join(REPORTS_PATH, "fraud_test_suite_textual.html")
        ...
        <body>
            <div class="container">
                ...
                <div class="footer">
                    ...
                    <p>🌤️ Pipeline Fraud Detection • Powered by Evidently AI</p>
                </div>
            </div>
        </body>
        </html>
...
# ========================== DÉFINITION DU DAG ==========================
with DAG(
    dag_id='fraud_detection_01_evidently_data_quality',
    default_args=default_args,
    description='Rapport Drift visuel + Test Suite textuelle sans graphiques pour les données fraud',
    schedule=None,
    catchup=False,
    tags=['evidently', 'data-quality', 'fraud'],
) as dag:
    task_download_csv = PythonOperator(
        task_id='download_fraud_csv',
        python_callable=download_fraud_csv_from_url
    )
    ...
    task_send_email = S3ReportEmailOperator(
        ...
        subject="- 𖣘 - Airflow 𓏺〢 Evidently : Data Quality Fraud",
        conn_id="smtp_default"
    )

    task_trigger_xgboost_dag = TriggerDagRunOperator(
        task_id='trigger_xgboost_dag',
        trigger_dag_id="fraud_detection_02_xgboost_dag",
        conf={
            "fraud_csv_filename": "{{ ti.xcom_pull(task_ids='download_fraud_csv', key='local_fraud_csv') }}"
        }
    )

    task_download_csv >> task_evidently_check >> task_upload_reports >> task_send_email >> task_trigger_xgboost_dag
